  ...
  return (
    <footer className="bg-gray-100 pt-10">
      <div className="max-w-7xl mx-auto px-5 grid grid-cols-1 md:grid-cols-4 gap-8">
        {/* Contact Us */}
        <div>
          <h3 className="text-xl md:text-2xl lg:text-3xl font-bold text-[#ED5E24] mb-3 underline">
            CONTACT US
          </h3>
          <div className="font-roboto text-sm md:text-base lg:text-lg">
            <p className="flex items-start gap-2">
              <img src={maps} className="w-5 md:w-6 lg:w-7" alt="" />
            ...
            <p className="mt-3 flex items-center gap-2">
              <img src={call_org} className="w-5 md:w-6 lg:w-7" alt="" />
              <span>+91 95581 37093</span>
            </p>
            <p className="mt-3 flex items-center gap-2">
              <img src={email_org} className="w-5 md:w-6 lg:w-7" alt="" />
              <span>info@icubeelevators.com</span>
            </p>
          </div>
        </div>
        ...
        <div>
          ...
          <ul className="text-sm md:text-base lg:text-lg space-y-2 font-roboto">
            {usefulLinks.map((link, index) => (
              <li
                key={index}
                className="hover:text-[#ED5E24] cursor-pointer flex items-center"
              >
                <img src={down_arrow_org} alt="arrow" className="h-3 md:h-4 lg:h-5 mr-2" />
                {link}
              </li>
            ))}
          </ul>
        ...
        <div>
          ...
          <iframe
            className="w-full h-40 md:h-48 lg:h-56 border rounded-lg"
      ...
      {/* Bottom Section */}
      <div className="bg-[#ED5E24] text-white flex justify-between items-center text-center md:px-10 py-4 mt-6">
        <p className="text-xs md:text-sm lg:text-base">
          Copyright © 2015-2024 By I-CUBE Parking Elevators
        </p>
        <div className="flex justify-between space-x-4 pr-8 mb-2">
          {[facebook_org, twitter_org, instagram_org, linkedin_org].map((icon, index) => (
            <img
              key={index}
              className="hover:bg-white/80 bg-white p-1 rounded-lg w-6 md:w-8 lg:w-10"
              src={icon}
              alt=""
            />
          ))}
